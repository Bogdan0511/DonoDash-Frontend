<template>
    <div class="containment">
        <router-link to="/" style="text-decoration: none;">
            <v-btn icon="mdi-home-circle-outline" size="x-large" variant="elevated" color="#25B7A1"
        ...
        <div class="login-form">
            ...
            <img src="@/assets/D_Logo.png" width="60px" height="60px"
                style="margin-bottom: 10px; border-radius: 100px;"></img>
            ...
            <v-card class="content">
                ...
                <v-text-field prepend-inner-icon="mdi-email-outline" variant="solo" class="donodash-input"
                    density="compact" rounded v-model="email"></v-text-field>

                <div class="input-type">Password:</div>

                <v-text-field :append-inner-icon="visible ? 'mdi-eye-off' : 'mdi-eye'"
                    :type="visible ? 'text' : 'password'" prepend-inner-icon="mdi-lock-outline" variant="solo"
                    density="compact" class="donodash-input" rounded v-model="password"
                    @click:append-inner="visible = !visible"></v-text-field>

                <v-btn size="large" rounded="xl" class="donodash-orange-button" @click="login">
                    LOGIN
                </v-btn>
            ...
            <div style="display: flex; flex-direction: row;">
                ...
                <router-link to="/register" style="text-decoration: none;">
                    <v-btn variant="text" class="register-button">
                        Register
                    </v-btn>
                </router-link>
...
export default {
    ...
    methods: {
        async login() {
            ...
            this.errorMessage = ''; 
            try {
                const response = await axios.post('http://localhost:8080/donodash/auth/login', {
                    email: this.email,
