<template>
    <v-container fluid class="content">
      ...
      <v-row>
        <v-col cols="12" md="6">
          <v-card class="pa-4" style="height: 300px; background-color: #25B7A1;">
            <h3 class="some-text">Top sources:</h3>
            <div v-for="(source, index) in topSources" :key="index" class="source-item">
              <v-row align="center">
                <v-col cols="2">
                    <div class="source-logo" :style="{ 'background-image': 'url(' + getSourceIcon(source.source) + ')' }"></div>
                </v-col>
                <v-col cols="7">
        ...
        <v-col cols="12" md="6">
          <v-card class="pa-4" style="height: 300px; background-color: #25B7A1;">
            ...
            <div style="height: 200px; display:flex; flex-direction: column; justify-content: space-between; margin-top: 25px;">
  ...
  export default {
    ...
    methods: {
      ...
      calculateMilestones() {
        ...
        this.donations.forEach(donation => {
          const month = new Date(donation.date_received).toLocaleString('default', { month: 'long' });
        ...
        const bestMonth = Object.entries(monthlyTotals)
          .reduce((best, [month, amount]) => amount > best.amount ? { month, amount } : best, { month: '', amount: 0 });
