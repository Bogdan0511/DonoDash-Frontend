      ...
      >
        <v-card
          :class="[
            ...
            'donor-card',
            {
              'top-contributor': index < 3,
              gold: index === 0,
              silver: index === 1,
              bronze: index === 2,
              'normal-contributor': index >= 3,
            },
          ]"
        ...
        >
          <div
            :class="{
              'donor-content': index >= 3,
              'top-donor-content': index < 3,
            }"
          >
            <div v-if="index < 3" class="donor-top-info">
              <div>
                <span class="donor-rank" style="flex: 1">{{ index + 1 }}.</span>
                {{ donor.name }}
              </div>
              <div class="donor-amount" style="flex: 1">
                {{ donor.totalAmount.toFixed(2) }} RON
              </div>
              <div class="donor-donations" style="flex: 1">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      sortedDonors() {
        return this.aggregatedDonors.sort(
